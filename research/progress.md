...
## 2026-09-24 13:00 — 阶段收尾与交接

**现状**：P0、P1（GEMM × decode 5 对的完整 3×2）、P4（prefill × decode 8 对，与 POD 同条件对比）、极限测试均已完成、验收并推送（最后一个实验提交 bcaecbb8）。GPU 空闲，无后台任务。工作区只有 TVM 子模块的 Z3 补丁（已存为补丁文件）。

**结论**：见 [summary_2026-09-24.md](summary_2026-09-24.md)。D1 的完整 / 弱化主张 0/13；在本卡（600W 功耗墙）上 kernel 内编排在最优划分下不胜过 green 分区，极限测试确认这是功耗墙造成的物理上限（功耗受限时同 SM 共置最多 +3.5%，功耗不受限时 +8–28%）；CoKernel 的价值是对划分的鲁棒性；L2 evict-first 是一个只在共置时才有价值的实现选择，但与编排方式正交。

**用户决定（13:00）**：暂时拿不到 H100 / B200；D1 之后的方向先考虑，"先整理到目前的结果，之后换到别的 session 进一步探索"。因此本会话停止启动新实验，只做整理。

**关注的问题 / 交接要点**：新会话接手时先读 `summary_2026-09-24.md` 的第 4 节（平台与测量方法、GPU 共享规则）与第 7 节（候选方向）；D1 定下之后按规则 6 存档并修订 proposal / plan。
