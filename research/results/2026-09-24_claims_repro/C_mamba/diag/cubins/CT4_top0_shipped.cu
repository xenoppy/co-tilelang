  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(x_desc, mbarrier[k], (&(((half_t*)x_shared)[(k * 4096)])), 0, (((((int)blockIdx.z) >> 6) * 256) + (k * 64)), ((int)blockIdx.x), (((int)blockIdx.z) & 63));
      }
      mbarrier[(k + 20)].wait(1);
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[(k + 4)].arrive_and_expect_tx(128);
        tl::tma_load((&(((half_t*)dA_cumsum_shared)[(k * 64)])), (&(dA_cumsum[(((((((int)blockIdx.z) & 63) * 131072) + (((int)blockIdx.x) * 2048)) + ((((int)blockIdx.z) >> 6) * 256)) + (k * 64))])), mbarrier[(k + 4)], 128);
      }
      mbarrier[(k + 24)].wait(1);
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[(k + 8)].arrive_and_expect_tx(128);
        tl::tma_load((&(((half_t*)dt_shared)[(k * 64)])), (&(dt[(((((((int)blockIdx.z) & 63) * 131072) + (((int)blockIdx.x) * 2048)) + ((((int)blockIdx.z) >> 6) * 256)) + (k * 64))])), mbarrier[(k + 8)], 128);
      }
      ...
      if (tl::tl_shuffle_elect<128>()) {
        ...
        #pragma unroll
        for (int i = 0; i < 2; ++i) {
          tl::tma_load(B_desc, mbarrier[(k + 12)], (&(((half_t*)B_shared)[((k * 8192) + (i * 4096))])), (i * 64), (((((int)blockIdx.z) >> 6) * 256) + (k * 64)), 0, (((int)blockIdx.z) & 63));
        }
      }
    }
  } else {
