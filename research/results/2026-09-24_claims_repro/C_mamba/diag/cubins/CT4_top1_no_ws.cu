...
extern "C" __global__ void __launch_bounds__(128, 1) chunk_state_fwd_kernel(const half_t* __restrict__ B, half_t* __restrict__ Output, const half_t* __restrict__ dA_cumsum, const half_t* __restrict__ dt, const half_t* __restrict__ x) {
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    __syncthreads();
    if (((int)threadIdx.x) < 64) {
      ((half_t*)dA_cumsum_shared)[((k * 64) + ((int)threadIdx.x))] = dA_cumsum[(((((((((int)blockIdx.z) & 63) * 131072) + (((int)blockIdx.x) * 2048)) + ((((int)blockIdx.z) >> 6) * 256)) + (k * 64)) + ((int)threadIdx.x)) + 128)];
    }
    tl::cp_async_commit();
    tl::cp_async_wait<7>();
    __syncthreads();
    ...
    for (int i_9 = 0; i_9 < 4; ++i_9) {
      tl::cp_async_gs<16>((&(((half_t*)x_shared)[((((((k * 4096) + (i_9 * 1024)) + ((((int)threadIdx.x) >> 3) * 64)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8))])), (&(x[(((((((((((int)blockIdx.z) & 63) * 8388608) + ((((int)blockIdx.z) >> 6) * 1048576)) + (k * 262144)) + (i_9 * 65536)) + ((((int)threadIdx.x) >> 3) * 4096)) + (((int)blockIdx.x) * 64)) + ((((int)threadIdx.x) & 7) * 8)) + 524288)])));
    }
    tl::cp_async_commit();
    #pragma unroll
    ...
    for (int i_13 = 0; i_13 < 8; ++i_13) {
      tl::cp_async_gs<16>((&(((half_t*)B_shared)[(((((((k * 8192) + (((((int)threadIdx.x) & 15) >> 3) * 4096)) + (i_13 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))])), (&(B[(((((((((int)blockIdx.z) & 63) * 262144) + ((((int)blockIdx.z) >> 6) * 32768)) + (k * 8192)) + (i_13 * 1024)) + (((int)threadIdx.x) * 8)) + 16384)])));
    }
    tl::cp_async_commit();
  }
  tl::cp_async_wait<7>();
