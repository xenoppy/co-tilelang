...
extern "C" __global__ void __launch_bounds__(128, 1) chunk_state_fwd_kernel(const half_t* __restrict__ B, half_t* __restrict__ Output, const half_t* __restrict__ dA_cumsum, const half_t* __restrict__ dt, const half_t* __restrict__ x) {
  ...
  __syncthreads();
  if (((int)threadIdx.x) < 64) {
    ((half_t*)dA_cumsum_shared)[(((int)threadIdx.x) + 192)] = dA_cumsum[((((((int)blockIdx.x) * 2048) + (((int)blockIdx.z) * 256)) + ((int)threadIdx.x)) + 192)];
  }
  tl::cp_async_commit();
  if (((int)threadIdx.x) < 64) {
    ((half_t*)dt_shared)[(((int)threadIdx.x) + 192)] = dt[((((((int)blockIdx.x) * 2048) + (((int)blockIdx.z) * 256)) + ((int)threadIdx.x)) + 192)];
  }
  tl::cp_async_commit();
  __syncthreads();
  #pragma unroll
  for (int i_7 = 0; i_7 < 4; ++i_7) {
    tl::cp_async_gs<16>((&(((half_t*)x_shared)[((((((i_7 * 1024) + ((((int)threadIdx.x) >> 3) * 64)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 12288)])), (&(x[((((((((int)blockIdx.z) * 1048576) + (i_7 * 65536)) + ((((int)threadIdx.x) >> 3) * 4096)) + (((int)blockIdx.x) * 64)) + ((((int)threadIdx.x) & 7) * 8)) + 786432)])));
  }
  tl::cp_async_commit();
  #pragma unroll
  for (int i_8 = 0; i_8 < 8; ++i_8) {
    tl::cp_async_gs<16>((&(((half_t*)B_shared)[(((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_8 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 24576)])), (&(B[((((((int)blockIdx.z) * 32768) + (i_8 * 1024)) + (((int)threadIdx.x) * 8)) + 24576)])));
  }
  tl::cp_async_commit();
  tl::cp_async_wait<15>();
  __syncthreads();
