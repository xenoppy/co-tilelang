...
extern "C" __global__ void __launch_bounds__(256, 1) chunk_state_fwd_kernel(__grid_constant__ const CUtensorMap B_desc, half_t* __restrict__ Output, const half_t* __restrict__ dA_cumsum, const half_t* __restrict__ dt, __grid_constant__ const CUtensorMap x_desc) {
  ...
  void* dA_cumsum_shared = ((void*)((char*)buf_dyn_shmem + 49152));
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 8; ++k) {
      mbarrier[((k & 3) + 16)].wait(((k >> 2) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[(k & 3)].arrive_and_expect_tx(4096);
        tl::tma_load(x_desc, mbarrier[(k & 3)], (&(((half_t*)x_shared)[((k & 3) * 2048)])), 0, ((((int)blockIdx.z) * 256) + (k * 32)), ((int)blockIdx.x), 0);
      }
      mbarrier[((k & 3) + 20)].wait(((k >> 2) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[((k & 3) + 4)].arrive_and_expect_tx(64);
        tl::tma_load((&(((half_t*)dA_cumsum_shared)[((k & 3) * 32)])), (&(dA_cumsum[(((((int)blockIdx.x) * 2048) + (((int)blockIdx.z) * 256)) + (k * 32))])), mbarrier[((k & 3) + 4)], 64);
      }
      mbarrier[((k & 3) + 24)].wait(((k >> 2) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[((k & 3) + 8)].arrive_and_expect_tx(64);
        tl::tma_load((&(((half_t*)dt_shared)[((k & 3) * 32)])), (&(dt[(((((int)blockIdx.x) * 2048) + (((int)blockIdx.z) * 256)) + (k * 32))])), mbarrier[((k & 3) + 8)], 64);
      }
      mbarrier[((k & 3) + 28)].wait(((k >> 2) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[((k & 3) + 12)].arrive_and_expect_tx(8192);
        #pragma unroll
        for (int i = 0; i < 2; ++i) {
          tl::tma_load(B_desc, mbarrier[((k & 3) + 12)], (&(((half_t*)B_shared)[(((k & 3) * 4096) + (i * 2048))])), (i * 64), ((((int)blockIdx.z) * 256) + (k * 32)), 0, 0);
        }
      }
    }
  } else {
    ...
    for (int k_1 = 0; k_1 < 8; ++k_1) {
      ...
      mbarrier[((k_1 & 3) + 20)].arrive();
      mbarrier[((k_1 & 3) + 8)].wait((k_1 >> 2));
      #pragma unroll
