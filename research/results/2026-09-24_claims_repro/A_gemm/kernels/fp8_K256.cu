...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap C_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 4; ++k) {
      mbarrier[((k & 1) + 2)].wait(((k >> 1) ^ 1));
      if (tl::tl_shuffle_elect<128>()) {
        mbarrier[(k & 1)].expect_transaction(8192);
        tl::tma_load(A_desc, mbarrier[(k & 1)], (&(((fp8_e4_t*)A_shared)[((k & 1) * 8192)])), (k * 64), (((int)blockIdx.y) * 128));
        mbarrier[(k & 1)].arrive_and_expect_tx(16384);
        tl::tma_load(B_desc, mbarrier[(k & 1)], (&(((fp8_e4_t*)B_shared)[((k & 1) * 16384)])), (k * 64), (((int)blockIdx.x) * 256));
      }
    }
  } else {
      ...
      mbarrier[(k_1 & 1)].wait((k_1 >> 1));
      {
        ...
        for (int ki = 0; ki < 2; ++ki) {
          ...
          for (int i_1 = 0; i_1 < 4; ++i_1) {
            tl::ptx_ldmatrix_x4((&(((fp8_e4_t*)A_shared)[(((((((k_1 & 1) * 8192) + (((((int)threadIdx.x) & 63) >> 5) * 4096)) + (i_1 * 1024)) + ((((int)threadIdx.x) & 15) * 64)) + (((((((int)threadIdx.x) & 7) >> 2) + ki) & 1) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16))])), (&(A_local[(i_1 * 16)])));
          }
          #pragma unroll
          for (int i_2 = 0; i_2 < 4; ++i_2) {
            tl::ptx_ldmatrix_x4((&(((fp8_e4_t*)B_shared)[((((((((k_1 & 1) * 16384) + ((((((int)threadIdx.x) >> 6) + 2) & 3) * 4096)) + (i_2 * 1024)) + (((((int)threadIdx.x) & 31) >> 4) * 512)) + ((((int)threadIdx.x) & 7) * 64)) + (((((((int)threadIdx.x) & 7) >> 2) + ki) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16))])), (&(B_local[(i_2 * 16)])));
          }
