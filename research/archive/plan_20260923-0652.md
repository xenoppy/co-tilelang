...
> v0.3（2026-09-22；v0.2 按 proposal v0.2 的 3×2 设计修订 P1 的度量、基线与 D1；v0.3 按 P1-S 的发现修订测量协议 §1.2 与任务列表 §9；旧版在 archive/）。研究内容见 [proposal.md](proposal.md)；进展与决策记录在 [progress.md](progress.md)，规则见 [rules.md](rules.md)。
...
- 环境约束（2026-09-22 核实）：CUDA 12.9 在 `/usr/local/cuda-12.9`，不在 PATH；当前用户没有免密 sudo，**无法用 `nvidia-smi -lgc` 锁频**；根分区只剩约 12GB，编译产物与实验数据要节省空间（原始数据只保留汇总所需部分）。
...
- **锁频**：能锁频时锁定 SM 频率（例如之前 MPK 实验用的 2800 MHz）。当前没有 sudo 权限，默认不锁频：测量期间用 NVML 每 10ms 采样功耗与实际 SM 频率，报告频率分布；同一组对比的各点交错测量（而不是一组测完再测下一组），以抵消温度与降频漂移。
- **L2 flush 必须"先写后读"**（v0.3）：只写的 flush 让 L2 充满脏行，被测 op 要为回写付费（RMSNorm 最多慢 30%）；只读的 flush 在本卡上清不干净。共跑加速比一律对照同一测量中交错执行的 serial 变体，而不是两个单跑时间之和。
- **稳态模式是共跑比较的主模式**（v0.3）：flush 模式的空闲阶段给功耗控制器留出余量，单跑 GEMM 运行中可达 608–689W，而持续 serving 时功耗墙一直生效。共跑的各变体（串行、双流、green context、CoKernel）都在持续负载、输入轮换（> 2×L2）下背靠背执行，报告每轮平均时间；flush 模式作为辅助。
- **温度**：功耗受限的 GEMM 在空闲 2 秒后快 2–3%；测量期间保持 GPU 持续负载，各变体交错测量。
...
每个配置记录资源签名（从编译产物读取寄存器、shared memory、实际线程数；sm_120 上 TileLang 可能自动改线程数）和数值等级（E0/E1）。

三档实现集合（对应 proposal 第 4 节）：
- **solo**：单跑最快的配置（同时记录 cuBLAS / 参考库的单跑时间，确认 TileLang 实现不弱）；
- **C_lib**：单跑 Pareto 前沿（单跑时间 × 资源向量：smem、寄存器×线程、CTA 数），加上用 green context 把 SM 数限制为 1/2、1/4 时各自单跑最快的配置；
- **C_derived**：C_lib 加上以共同资源契约为条件推导出的变体。契约 = (线程数, 每 CTA shared memory 预算, 寄存器上限)，取自伙伴的候选；对每个契约，按语义重新推导该算子在契约内的最优若干配置（例如对齐到 256 线程时让 decode 每 CTA 处理两倍的 head；给伙伴留出 X KB shared memory 时减少流水级数）。
...
- **M2**：green context SM 分区，划分比例以 8 个 SM 为步长扫描。每个分区内的实现从对应档位的集合中挑选（lib 档包含 1/2、1/4 预算下调优的实现，即 GOLDYLOC 式）。
- **M3**：手写 CoKernel 原型。
  - P1 做全部三种绑定层级：先 SM 级（最简单，用来验证分派与测量），再 CTA 级（POD 式），最后 warp 级（顺带验证 `ThreadPartialSyncRewriter` 生成的 named barrier）。
  - P2 至少做 SM 级和 CTA 级。
  - 资源对齐的两个对照：多余线程空转（Rammer 式）；虚拟 CTA（POD 式，一个 CTA 串行执行多个伙伴 tile）。
  - 先测"另一角色编进 kernel 但不执行"的开销（寄存器按最大值耦合、指令 cache、分派）：它决定 CTA 级共驻在 64K 寄存器 / 100KB smem 的 SM 上是否可行，要在大规模测量之前知道。
- **M4（P4 对，POD 的场景）**：FlashInfer POD。先用 `%smid` 探测确认本机 `%nsmid` 与 SM 编号是否连续（FlashInfer 按 SM 数开计数器数组，`%nsmid` 大于 SM 数时会越界），安全后再作为 kernel 内编排的现成基线。

手写原型同时是 P2 编译器的"金标准"，也用来确定 TileLang 需要补哪些能力。

### 2.5 要算的量

按 proposal 第 4 节的 3×2 设计计算：
- **T[solo, inter]**：单跑最优配置在 M1/M2 下的最好结果。
- **T[lib, inter]**、**T[derived, inter]**：分别在 C_lib、C_derived 的配置乘积空间上穷举 M1、M1+、M2，取最好结果。这些组合复用已编译的 kernel，只需编译 N_A + N_B 个。
- **T[solo, intra]**：把单跑最优配置放进 CoKernel（只做放进同一 kernel 所必需的最小调整），取最好编排下的结果。
- **T[lib, intra]**：在 C_lib（每个算子 10–15 个）的乘积空间上穷举 CoKernel。线程数不一致时用两种对齐方式（多余线程空转、虚拟 CTA）中较好的一种。
- **T[derived, intra]**：在 C_derived 上穷举 CoKernel。
- **参照**：T_serial、LB。

最强的跨 kernel 基线记为 T_inter* = min(T[solo, inter], T[lib, inter], T[derived, inter])。

### 2.6 归因分析

对 T[lib, ·]、T[derived, ·] 选中的配置，报告它在单跑排名中的位置、单跑慢了多少，并借助资源签名和 Nsight 数据归因到具体效应：SM 内容量、波次、资源耦合、DRAM/L2 争用、pipe 争用。挑 2–3 个典型案例做完整剖析。
...
- 表 1：收益归因；
- 阶段小结写入 [progress.md](progress.md)。

### 2.8 决策点 D1

（2026-09-22 按 proposal v0.2 修订，仍在看到任何 P1 数据之前。）
- **继续（完整主张）**：P1–P4 中至少 2 对同时满足：
  - T_inter* / T[derived, intra] ≥ 1.10（相对最强跨 kernel 基线的总收益）；
  - T[lib, intra] / T[derived, intra] ≥ 1.05（重新推导实现的价值，核心主张）。
- **继续（弱化主张）**：至少 2 对满足 T_inter* / T[lib, intra] ≥ 1.10，但重新推导的价值在所有算子对上都 < 5%。此时贡献改为"统一的编排空间 + 按算子对自动选择绑定层级与调度"，重新推导降为消融项；需要重新评估与 Rammer、POD 的区分度，并修订 proposal。
...
- `research/`：proposal.md、plan.md、progress.md、rules.md；旧版本存于 `research/archive/`；
- `research/bench/`：测量框架、压力 kernel、探测 kernel；
...
- [x] 编译 co-tilelang（sm_120），跑通示例（GQA decode 需换小配置）
- [x] TileLang 能力调研（notes/tilelang_capability_survey.md）
- [x] 相关工作扫描（notes/related_work_scan.md），proposal v0.2 / plan v0.2
- [x] 测量框架 v0（cobench）与 `%smid` 探测
- [x] FlashInfer 基线（decode、prefill、POD；发现 POD 的流 / graph bug 与功耗墙效应）→ proposal v0.3
- [x] 算子库 v0（`cotile/ops/`）：118 个配置，grid / 持久化版逐位相同，资源签名
- [x] CoKernel 构建器 v0（SM / CTA 级绑定，静态 / 动态调度，接手，smem 复用的核心改动）
- [x] 单跑 profiling 与 C_lib 目录（`cotile/catalog.py`），配对表
- [ ] 测量方法 v1：先写后读的 flush、稳态共跑模式、统一的 GPU 守卫（pmon 规则）、持久化静态调度惩罚的根因
- [ ] P1 研究点的 3×2 测量（GEMM × decode：GEMM 4096³ × B16×8192 为主，另做时长比扫描）：M0/M1/M2/M4（POD 仅适用于 P4）与 CoKernel 的 solo / lib 档
- [ ] 按共同资源契约推导的 derived 变体（先手工：对齐线程数、限制寄存器、降低 smem、低能耗大 tile）
- [ ] P2 研究点（GEMM × RMSNorm）的 3×2 测量
- [ ] 压力 kernel v0（P3 的敏感度刻画）
