...
> v0.2（2026-09-22；按 proposal v0.2 的 3×2 设计修订 P1 的度量、基线与 D1；旧版在 archive/）。研究内容见 [proposal.md](proposal.md)；进展与决策记录在 [progress.md](progress.md)，规则见 [rules.md](rules.md)。
...
- [x] 编译 co-tilelang（sm_120），跑通 gemm、GQA decode、RMSNorm、warp_specialize 示例（GQA decode 需换小配置）
- [x] TileLang 能力调研（notes/tilelang_capability_survey.md）
- [x] 相关工作扫描（notes/related_work_scan.md），据此修订 proposal v0.2 / plan v0.2
- [ ] 测量框架 v0：event 计时、CUDA Graph、输入轮换、NVML 采样（无法锁频）、green context 流
- [ ] `%smid` 探测 kernel（含 `%nsmid`、编号空洞、green context 下的 SM 集合）
- [ ] 算子库 v0（`cotile/ops/`）：GEMM、decode attention、RMSNorm 的 tile_body macro、普通版与持久化版、30–50 个配置、资源签名
- [ ] 单跑 profiling：各配置的单跑时间、1/2 与 1/4 SM 预算下的时间，构造 solo / C_lib；cuBLAS 与 torch 参照
- [ ] "另一角色编进来但不执行"的开销测量（GEMM × decode 的 SM 级 / CTA 级原型）
- [ ] P1 的 SM 级 CoKernel 手写原型
- [ ] 压力 kernel v0：DRAM 流、Tensor Core 满载、SM 资源占位
- [ ] P1、P2 两对在 M1、M1+、M2 下的配置乘积空间共跑
- [ ] FlashInfer POD 在 sm_120 上的可用性（依赖 `%nsmid` 探测结果）
