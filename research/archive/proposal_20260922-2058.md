...
> 研究提案草稿 v0.2（2026-09-22；v0.1 存档于 archive/。v0.2 依据 [相关工作扫描](notes/related_work_scan.md) 修订了 §1.3、§4、§6–§9 和参考文献）。执行计划见 [plan.md](plan.md)，进展与决策记录见 [progress.md](progress.md)。
...
与已有工作相比，本研究的核心主张是：**为共同的资源契约（CTA 形状、shared memory 预算、寄存器上限）从语义重新推导实现，能带来"从单跑调优版本中挑选"得不到的收益**；并且 POD、HFuse、Rammer、MPK 等已有设计都是同一个编排空间里的点，可以由搜索按算子对自动选出。
...
详细对比见第 6 节与 [notes/related_work_scan.md](notes/related_work_scan.md)。这里只列出已经被做过、因而本研究不能再声称的部分：

- **在一个 kernel 里共置计算密集与访存密集的操作，并为此改实现**：POD-Attention [1] 为 prefill/decode attention 手工缩小 decode tile、选择每 SM 2 或 4 个 CTA、引入虚拟 decode CTA，并读 `%smid` 做 SM 感知的 CTA 调度（已进入 FlashInfer）。它只针对一个算子对。
- **各绑定层级的机制本身**：CTA 级 `%smid` 绑定（POD）；warp 级按线程区间拼接并搜索线程划分与寄存器上限（HFuse [2]）；SM 级持久化 worker、静态或动态队列、按 SM 交错不同类型的任务、paged shared memory（MPK [5]、Event Tensor [19]、Hazy megakernel [20]）；运行时自适应的角色 SM 划分（mKernel [21]）；多个异构持久化 block 驻留同一 SM、线程数取最大值、多余线程提前退出（Rammer [16]）。
- **按共同执行语境在已有实现版本中挑选**：kernel 内有 Rammer（每个算子多个 rKernel 版本，编译期按 profiling 选择），kernel 间有 GOLDYLOC [22]（在 1/2、1/4 GPU 预算下重新调优 GEMM，挑出共跑最优版本）、NanoFlow [3]（枚举线程块数、warp 数、tile 大小，两两干扰 profiling + MILP）、VELTAIR [23]（CPU 上按干扰等级多版本编译）。"单跑最优 ≠ 共跑最优"这一现象本身已被报告。
- **从语义搜索实现**：Mirage [4]、Souffle [24]（包括独立张量表达式的水平合并），但目标都是单跑；MPK 把 Mirage 搜到的实现放进 megakernel，同样按单跑目标。

尚未有人做的是：**(a) 在两个角色必须共享的资源契约下，从语义为每个角色重新推导 tile 程序**（而不是从单跑调优库里挑、让多余线程空转、或像 POD 那样手工平衡）；**(b) 用一个表示与搜索空间覆盖 SM/CTA/warp 三级绑定 × 静态/动态/优先级调度 × 跨角色接手，让已有设计都成为其中的点，并按算子对自动选择**；(c) 在可控成本下联合搜索实现与编排；(d) 与伙伴无关的数值结果（partner-invariance）。本研究不把"共置有收益"或"为共置改实现有收益"当作创新。
...
- **RQ1（价值与来源）**：与"各自单跑最优实现 + 最好的共置机制"相比，联合选择实现与 kernel 内编排能带来多少收益？其中有多少只能靠"为共同资源契约重新推导实现"得到，而无法靠"从单跑调优版本中挑选"（Rammer / GOLDYLOC 可达）得到？收益来自哪些资源效应（SM 内容量、波次、资源耦合、共享带宽、流水线争用）？
...
- **solo**：各自单跑最优的实现；
- **lib**：从单跑调优库 C_lib 中联合挑选。C_lib(X) 取 X 在"单跑时间 × 资源向量"上的 Pareto 前沿，加上在 1/2、1/4 SM 预算下单跑调优得到的最优实现（GOLDYLOC 式）。这是 Rammer、GOLDYLOC、NanoFlow 一类方法能达到的范围；
- **derived**：在完整实现空间 C(X) 中联合搜索。C(X) 从语义推导，包括以伙伴的资源形状为条件的变体（共同线程数、shared memory 预算、寄存器上限下重新 tile，例如 256 线程下 decode attention 每个 CTA 处理两倍的 head）。这些变体由伙伴施加的约束决定，单跑调优（即使在缩小的 SM 预算下）一般不会选出它们；两者的差别需要实测，而不是假定。

与两种编排方式组成 3×2 设计：

| | 跨 kernel 共置（多 stream / green context 分区） | kernel 内编排（CoKernel） |
|---|---|---|
| solo | T[solo, inter] | T[solo, intra] |
| lib | T[lib, inter] | T[lib, intra] |
| derived | T[derived, inter] | T[derived, intra] |

- T[solo, inter] / T[lib, inter]：按共置语境挑选实现的价值（已有工作覆盖，作为参照）；
- T[lib, inter] / T[lib, intra]：kernel 内编排本身的价值；
- **T[lib, intra] / T[derived, intra]：重新推导实现的价值，是本研究的核心主张**；
- T[lib, inter] / T[derived, inter]：重新推导在跨 kernel 共置下的价值（交互项的参照：若它远小于上一项，说明重新推导的收益依赖 kernel 内编排）；
- 另报串行时间 T_serial，以及资源下界 LB = max_r (d_A,r + d_B,r) / cap_r。

另设两个资源对齐基线，作为 intra 列的对照：让多余线程空转（Rammer 式），以及虚拟 CTA（POD 式，一个 CTA 串行执行多个伙伴 tile 以对齐线程数）。
...
|---|---|---|---|---|
| SM 级 | 持久化 CTA 读 `%smid`，按 SM 编号决定角色 | 每 CTA 占用取两角色最大值 | L2、DRAM | `%smid` 可能不连续，`%nsmid` 可能大于实际 SM 数（PTX 规范如此；FlashInfer 的 POD 实现按 SM 数开计数器数组，在本机 188/192 SM 上可能越界），需要重映射 |
...
|---|---|---|---|---|---|
| POD-Attention [1]（及 FlashInfer 移植） | 否 | 是（手工：小 decode tile、2/4 CTA/SM、虚拟 CTA） | 是（CTA 级，`%smid` + 每 SM 票号，SM 内共驻） | 否（运行时在两种配置中选） | prefill × decode attention |
| HFuse [2] | 否 | 部分（线程数、寄存器上限；不改 tile 与分解） | 是（CTA 内按线程区间，named barrier） | 部分（线程划分 × 寄存器上限） | 任意两个 CUDA kernel |
| Tacker [7] | 否 | 否 | 是（Tensor Core 与 CUDA Core kernel 融合） | 部分（时长预测） | TC kernel + CUDA Core kernel |
| Rammer [16] | 否（kernel 库） | 部分（在多个库版本间挑选；多余线程提前退出） | 是（编译期静态 rProgram，多个 vEU 可驻留同一 SM） | 部分（profiling 引导的启发式） | DNN 推理的算子间并行 |
| Souffle [24] | 是（张量表达式级变换） | 否（单跑目标） | 否（合并成同构的单个 kernel） | 是 | DNN 子程序 |
| Mirage [4] | 是 | 不涉及共置 | 否（所有 CTA 执行同一段代码） | 是 | 多算子子图 |
| MPK [5] | 部分（任务实现由 Mirage 按单跑目标搜索） | 否 | 是（SM 级任务图；每 SM 一个 worker 顺序执行，仅下一任务预取与当前计算重叠） | 部分（只搜任务实现） | LLM 推理 |
| Event Tensor [19]、Hazy megakernel [20] | 否 | 否 | 是（静态 / 动态任务调度；按 SM 交错任务类型） | 否 | LLM megakernel |
| TileLink [25]、mKernel [21] | 否 | 否 | 是（计算 / 通信角色按 SM 划分；mKernel 运行时调整划分） | 否 / 部分 | 计算 × 通信 |
| GOLDYLOC [22] | 否 | 是（在缩小的 GPU 预算下重新调优，库） | 否（多 stream） | 是（调优 + 预测器） | 并发 GEMM（MI100） |
| NanoFlow [3] | 否 | 部分（线程块数、warp 数、tile 大小的变体） | 否（多 stream） | 是（两两干扰 profiling + MILP） | LLM serving 算子 |
| VELTAIR [23] | 部分（TVM 多版本） | 是（按干扰等级多版本） | 否 | 是 | CPU 多租户推理 |
| Bullet、Nexus、DuetServe、MuxWise 等 [26] | 否 | 否 | 否（SM 掩码 / green context） | 是（roofline 或估计器） | LLM prefill/decode serving |
| LithOS [27]、REEF [13]、Tally [14]、Orion [12]、KRISP [15]、MuxServe [18] | 否 | 否 | 否 / 部分（块级调度） | 部分 | 固定 kernel 的共享与调度 |
| Elastic Kernels [8] | 否 | 是（改 grid/block，通用变换） | 否 | 部分 | 通用 kernel |
| Warped-Slicer [9] | — | — | 硬件机制（SM 内资源划分） | — | 硬件 |

本研究落在这几列的交集：从语义推导实现空间，在 kernel 内做多层级的 tile 编排，以共同执行为目标自动搜索，并覆盖多类算子。与最接近的工作的区别：
- **POD-Attention**：它手工完成的"改 tile / CTA / 分解 + SM 感知调度"，在本研究中由编译器对任意算子对自动搜索完成；POD 的方案是这个空间里的一个点。审稿人的默认解读会是"自动化的 POD"，所以必须证明：搜索在 POD 自己的算子对上能追平 POD，在其他算子对上选出不同的绑定层级或实现，且重新推导的增益（3×2 中的 T[lib, intra] / T[derived, intra]）在至少部分算子对上显著。
- **Rammer**：它也让多个算子的 tile 共驻同一 SM，也在多个实现版本间按共同调度挑选，但版本来自固定的库，线程数不齐时让多余线程空转，编排是编译期静态的，也不建模 SM 内争用。本研究为共同的资源契约重新推导实现，建模争用，并支持动态调度与跨角色接手。
- **MPK / Event Tensor / Hazy megakernel**：它们有成熟的 kernel 内任务运行时，但每个 SM 同一时刻只执行一个任务（MPK 只允许下一任务的预取与当前计算重叠），实现按单跑目标确定。本研究改变的是实现本身，并允许异构 tile 在 SM 内共驻。这组工作的作者同时掌握语义搜索、kernel 内运行时和 SM 划分，是最主要的撞车风险（见第 9 节）。
- **GOLDYLOC / NanoFlow**：它们证明了"为共同执行调优实现"的价值，但停留在跨 kernel 并发与库内挑选；本研究的 lib 档实现即对应它们的能力范围，作为对照。

## 7. 预期贡献

1. **刻画研究**：在 Blackwell（sm_120）上系统量化单算子对共置中"按共置语境挑选实现"、"为共同资源契约重新推导实现"与"kernel 内编排"各自的收益及其资源来源（3×2 分解）。
...
4. **评估**：覆盖多种算子对和形状，把 POD、HFuse、Rammer、MPK 式 worker、green context 分区（含 GOLDYLOC 式预算调优）作为编排空间中的点复现为基线，证明搜索在它们各自的场景上能追平、在其他场景上更好，并做消融。
5. **与伙伴无关的数值结果**：E0/E1 分级与确定性模式（对应 batch invariance [28] 在共置场景下的类比），并量化其代价。
6. **长期**：扩展到子图级描述与编排，结合 Mirage 式结构搜索。
...
- **基线**：串行；多 stream；green context SM 分区（本机可用，粒度 8 个 SM），并配 GOLDYLOC 式在 1/2、1/4 SM 预算下调优的实现；多 stream + 共跑感知配置（持久化 grid、限制占用率）；HFuse 式融合；POD 式手工编排（FlashInfer POD，需先确认 `%nsmid` 问题）；Rammer 式静态编排 + 库内挑选 + 多余线程空转；MPK / Hazy 式 SM 级 worker + 动态队列。所有单跑参照同时报告 cuBLAS 与 TileLang 自身的最优实现，避免因 kernel 本身低效而虚增共置收益。
...
| 风险 | 表现 | 应对 |
|---|---|---|
| 收益空间小 | 3×2 分解中，按共置语境挑选或重新推导实现的价值普遍低于 5% | 第一阶段结束设决策点；转向收益集中的场景（单跑时长相当的计算 × 访存对、小批量占不满 GPU 的算子），或提前进入子图级 |
...
| 被读成"自动化的 POD" | 搜索结果大多是 CTA 级 + 单跑调优实现，重新推导的增益很小 | P1 就测 3×2 中的 T[lib, intra] / T[derived, intra]；若在所有算子对上都 < 5%，按 plan 的 D1 转为"统一编排空间 + 搜索"的弱化主张，或转向 |
| 撞车 | MPK / Mirage 与 Event Tensor 团队已具备语义搜索、kernel 内运行时和 SM 划分，"每 SM 两个共驻 worker"是它们的自然下一步 | 持续跟踪（mirage-project/mirage、Event Tensor、mKernel、FlashInfer）；把与之最不同的部分——共同资源契约下的重新推导、SM 内异构共驻、与伙伴无关的数值——放在最前面 |
...
5. X. Cheng et al. MPK: A Compiler and Runtime for Mega-Kernelizing Tensor Programs. arXiv 2512.22219, 2025（v2 2026-06，致谢 OSDI shepherd，很可能是 OSDI 2026）.
...
19. H. Jin et al. Event Tensor: A Unified Abstraction for Compiling Dynamic Megakernel. MLSys 2026（arXiv 2604.13327）.
20. B. Spector et al. Hazy Research megakernel 系列博客（"Look Ma, No Bubbles!" 2025-05；"We Bought the Whole GPU…" 2025-09）与 ThunderMLA（2025-03）.
21. Z. Mao et al. mKernel: Fast Multi-GPU, Multi-Node Fused Kernels. arXiv 2609.13585, 2026.
22. S. Pati, S. Aga, N. Jayasena, M. D. Sinclair. GOLDYLOC: Global Optimizations & Lightweight Dynamic Logic for Concurrency. ACM TACO 2025.
23. Z. Liu et al. VELTAIR: Towards High-Performance Multi-tenant Deep Learning Services via Adaptive Compilation and Scheduling. ASPLOS 2022.
24. C. Xia et al. Souffle: Optimizing Deep Learning Inference via Global Analysis and Tensor Expressions. ASPLOS 2024.
25. S. Zheng et al. TileLink: Generating Efficient Compute-Communication Overlapping Kernels using Tile-Centric Primitives. MLSys 2025.
26. Bullet（ASPLOS 2026）、Nexus（arXiv 2507.06608）、DuetServe（arXiv 2511.04791）、Drift/MuxWise（ASPLOS 2026）、semi-PD（arXiv 2504.19867）、SpaceServe（NeurIPS 2025）：基于 SM 分区的 prefill/decode 共置 serving 系统.
27. P. H. Coppock et al. LithOS: An Operating System for Efficient Machine Learning on GPUs. SOSP 2025.
28. Thinking Machines Lab. Defeating Nondeterminism in LLM Inference. 2025（batch invariance）.
